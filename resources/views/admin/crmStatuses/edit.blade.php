...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-statuses.update", [$crmStatus->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="user_id">{{ trans('cruds.crmStatus.fields.user') }}</label>
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $crmStatus->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <span class="text-danger">{{ $errors->first('user') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.crmStatus.fields.user_helper') }}</span>
            </div>
            <div class="form-group">
