...
@can('randevu_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.randevus.create') }}">
                ...
                <tbody>
                    @foreach($randevus as $key => $randevu)
                        <tr data-entry-id="{{ $randevu->id }}">
                            ...
                            <td>
                                @can('randevu_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.randevus.show', $randevu->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('randevu_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.randevus.edit', $randevu->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('randevu_delete')
                                    <form action="{{ route('admin.randevus.destroy', $randevu->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('randevu_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.randevus.massDestroy') }}",
