        ...
        <div class="col-md-12">
            @can('healty_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.healties.create') }}">
                            ...
                            <tbody>
                                @foreach($healties as $key => $healty)
                                    <tr data-entry-id="{{ $healty->id }}">
                                        ...
                                        <td>
                                            @can('healty_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.healties.show', $healty->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('healty_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.healties.edit', $healty->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('healty_delete')
                                                <form action="{{ route('frontend.healties.destroy', $healty->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('healty_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.healties.massDestroy') }}",
