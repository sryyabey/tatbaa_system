        ...
        <div class="col-md-12">
            @can('galery_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.galeries.create') }}">
                            ...
                            <tbody>
                                @foreach($galeries as $key => $galery)
                                    <tr data-entry-id="{{ $galery->id }}">
                                        ...
                                        <td>
                                            @can('galery_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.galeries.show', $galery->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('galery_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.galeries.edit', $galery->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('galery_delete')
                                                <form action="{{ route('frontend.galeries.destroy', $galery->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('galery_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.galeries.massDestroy') }}",
